...
class IRSystem:
    ...
    def compute_tfidf(self):
        ...
        for word in self.vocab:
            ...
            for d in posting:
                self.tfidf[word][d] = (1. + math.log10(tf[word][d])) * math.log10(N / df)
...
#                inv_index[word].add(i)
        ...
        for word in inv_index.keys():
            inv_index[word] = sorted(list(inv_index[word]))
    ...
    def boolean_retrieve(self, query):
        ...
        for i in xrange(len(query)):
            postings.append(self.get_posting(query[i]))
        ...
        while True:
            ...
            for i in xrange(1, len(query)):
                if postings[0][pos[0]] != postings[i][pos[i]]:
                    matched = False

            if matched:
                docs.append(postings[0][pos[0]])
                for i in xrange(len(query)):
                    pos[i] += 1
                ...
                minimum_index = 0
                for i in xrange(len(query)):
                    if postings[i][pos[i]] < postings[minimum_index][pos[minimum_index]]:
                        minimum_index = i
    ...
    def rank_retrieve(self, query):
        ...
        for word in set(query):
            query_vector[word] = 1. + math.log10(query_tf[word])

        query_norm = 0.0
        for word in set(query):
            query_norm += query_vector[word] ** 2
        ...
        for d, doc in enumerate(self.docs):
            doc_norm = 0.0
            for word in set(doc):
                doc_norm += self.get_tfidf(word, d) ** 2
            ...
            score = 0.
            for word in set(query):
                score += query_vector[word] * self.get_tfidf(word, d) 
                #print word + ":", self.get_tfidf(word, d),

            scores[d] = score / (query_norm * doc_norm)

            print d, scores[d], score, query_norm, doc_norm,  self.titles[d], query
        """
        query_tf = defaultdict(float)
        for word in query:
            query_tf[word] += 1.
        ...
        length = [0.] * len(self.docs)
        for d in range(len(self.docs)):
            for word in set(self.docs[d]):
                length[d] += self.get_tfidf(word, d) ** 2
            length[d] = math.sqrt(length[d])

        for word in set(query):
            posting_list = self.get_posting(word)
            for d in posting_list:
                scores[d] += query_vector[word] * self.get_tfidf(word, d)

        for d in xrange(len(self.docs)):
            scores[d] /= length[d]
            #print d, scores[d], length[d], self.titles[d]
